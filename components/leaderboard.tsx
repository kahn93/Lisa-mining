...
export function Leaderboard() {
  ...
  const mockPlayers: Player[] = [
    ...
    ...Array.from({ length: 45 }, (_, i) => ({
      ...
      miningPower: Math.floor(Math.random() * 30) + 5,
      questsCompleted: Math.floor(Math.random() * 10),
      achievements: Math.floor(Math.random() * 15),
      tonSpent: Math.random() * 10,
    ...
    <div className="space-y-6 pb-20">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground">
                {currentCategory && formatValue(currentPlayer[currentCategory.sortBy] as number, activeCategory)} •{" "}
      ...
      <Tabs value={activeCategory} onValueChange={setActiveCategory} className="w-full">
        ...
        <div className="space-y-2">
          {players.slice(0, 50).map((player, index) => (
            <Card key={player.id} className={`p-3 ${index < 3 ? "border-primary/30 bg-primary/5" : ""}`}>
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {currentCategory && formatValue(player[currentCategory.sortBy] as number, activeCategory)} •{" "}
      ...
      <Card className="p-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="text-center">
            <div className="text-xl font-bold text-primary">
              {currentCategory && Math.max(...players.map((p) => p[currentCategory.sortBy] as number)).toLocaleString()}
            </div>
          ...
          <div className="text-center">
            <div className="text-xl font-bold text-primary">
              {currentCategory &&
                Math.floor(
                  players.reduce((sum, p) => sum + (p[currentCategory.sortBy] as number), 0) / players.length,
                ).toLocaleString()}
            </div>
