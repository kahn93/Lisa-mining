  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="text-center">
        ...
        <p className="text-sm text-muted-foreground">Enhance your journey with premium items and upgrades</p>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="premium" className="space-y-4">
          <TonPaymentSystem onPaymentSuccess={handlePaymentSuccess} onPaymentError={handlePaymentError} />
        </TabsContent>
        ...
        <TabsContent value="owned" className="space-y-4">
          {purchasedItems.length === 0 ? (
            <Card className="p-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">Purchase premium items to see them here</p>
            </Card>
          ) : (
                ...
                <Card key={item.id} className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">TX: {item.transactionHash.slice(0, 10)}...</p>
                    </div>
          ...
          <div className="space-y-2">
            {purchasedItems.slice(-3).map((item) => (
              <div key={item.transactionHash} className="flex justify-between items-center text-sm">
                <span>{item.name}</span>
                <span className="text-muted-foreground">{item.purchaseDate.toLocaleDateString()}</span>
              </div>
            ))}
          </div>
        </Card>
