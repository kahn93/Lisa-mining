    ...
    const generateBaseMissions = (playerLevel: number): Mission[] => {
      ...
      const baseMissions: Mission[] = [
        {
          ...
          description: "Dark energy has corrupted the sacred grove. Defeat the shadow demons and restore the light.",
          type: "main",
          difficulty: playerLevel < 10 ? 1 : playerLevel < 20 ? 2 : 3,
          rewards: { divineEssence: 50 * difficultyMultiplier, experience: 100 * difficultyMultiplier },
          objectives: [
      ...
      sideMissionTemplates.forEach((template, index) => {
        baseMissions.push({
          ...
          type: "side",
          difficulty: Math.random() > 0.7 ? 3 : Math.random() > 0.4 ? 2 : 1,
          rewards: { divineEssence: 25 * difficultyMultiplier, experience: 50 * difficultyMultiplier },
  ...
  const handleDailyCheckIn = async () => {
    ...
    try {
      ...
      setDailyCheckIn((prev: { lastCheckIn: string; streak: number; }) => ({
        ...
        lastCheckIn: today,
        canCheckIn: false,
        streak: prev.lastCheckIn === new Date(Date.now() - 86400000).toDateString() ? prev.streak + 1 : 1,
        isProcessing: false,
      }))
