...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingPlayerIndex = airdropData.allocations.findIndex((p) => p.walletAddress === playerData.walletAddress)
    ...
    const totalPercentage = airdropData.allocations.reduce((sum, p) => sum + p.allocationPercentage, 0)
